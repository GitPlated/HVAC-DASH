...
// check that ALWAYS runs first (see wireIdentityGate's card-click handler)
// -- a lightweight password set here would never actually be checked for
// them, so this app's own password controls would be misleading, not just
// unnecessary. Kept in the list at their normal position (rather than
// pulled into a separate note) so nobody looks missing/forgotten, but with
// no input or buttons at all -- there's nothing here to disable-and-explain
// per control when the row does nothing to begin with.
function buildManagedElsewhereRow(identity) {
  ...
  note.className = "manage-user-row-elsewhere-note";
  note.textContent = "Managed on MM Dashboard — not editable here.";
